...
이벤트의 기본 단위는 큰 주제가 아니라 개별 사건/장면이다. 
가능하면 서로 다른 사건은 세밀하게 분리하라. 
일반적으로 하나의 이벤트가 {MIN_INTERVAL_SEC}초를 크게 넘도록 만들지 마라.
단, 하나의 사건이 실제로 계속 진행되는 경우에는 억지로 분리하지 않아도 된다. 
반대로 {MIN_INTERVAL_SEC}초 이내라도 새로운 사건, 강한 리액션, 재미있는 발언, 채팅과의 상호작용 등이 발생하면 별도의 이벤트로 분리하라. 
즉, 5분마다 이벤트를 하나 만드는 것이 목적이 아니다. 5분은 지나치게 큰 구간으로 묶지 않기 위한 참고 기준일 뿐이다. 
"""
CHAT_INSTRUCTION = """
